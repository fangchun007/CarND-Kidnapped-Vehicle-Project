 ...
 *      Author: Tiffany Huang & Chun Fang
 */
...
void ParticleFilter::init(double x, double y, double theta, double std[])
{
  ...
  num_particles = 136; // this number depends on the assumed distribution (Gaussian) and dimension of the used map (2D)
  cout << "In this particle filter implementation, we used " << num_particles << " particles to represent the distribution of the location of vehicle." << endl;
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate)
{
  ...
  {
    // Precalculate items
    double c_vy = velocity/yaw_rate;
    double angle_displacement = yaw_rate*delta_t;
    ...
    for (int i=0; i<num_particles; i++)
    {
      ...
      new_x = x0 + c_vy * (sin(theta0+yaw_rate*delta_t) - sin(theta0));
      new_y = y0 + c_vy * (cos(theta0) - cos(theta0+yaw_rate*delta_t));
      new_theta = theta0 + angle_displacement;
      // Add random Gaussian noise
      particles[i].x = new_x + dist_x(gen);
      particles[i].y = new_y + dist_y(gen);
